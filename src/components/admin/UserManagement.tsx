...
import { UserType } from "../../interfaces/Users"; // Ensure this includes `createdAt`
import { fetchUsers } from "../../controllers/user"; // Import the thunk to fetch users

const UserManagement = () => {
  const dispatch = useDispatch();
  
  // Retrieve the list of users from Redux store
  const users = useSelector((state: AppState) => state.user.usersList) as UserType[];
  
  // Local state for search query, sort option, and pagination
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {currentUsers.map((user) => (
              <tr key={user._id}>
                ...
                <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{user.phoneNumber}</td>
                <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{user.email}</td>
                <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{user.location || "N/A"}</td>
                <td className="whitespace-nowrap px-6 py-4">
                  <span className={`inline-flex rounded-full px-2 text-xs font-semibold leading-5 ${user.activeStatus ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
